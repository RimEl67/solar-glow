...
const projects = [
  ...
  },
  {
    ...
    img: 'https://images.unsplash.com/photo-1508514177221-188b1cf16e9d?auto=format&fit=crop&q=80&w=800',
    details: {
      fr: 'Installation photovoltaïque en toiture pour un établissement hôtelier de luxe à Ouarzazate. Ce projet permet d\'économiser plus de 40% sur la facture énergétique annuelle.',
      ar: 'تركيب ألواح كهروضوئية على السطح لفندق فاخر في ورزازات. يسمح هذا المشروع بتوفير أكثر من 40٪ من فاتورة الطاقة السنوية.'
    }
  },
]
    ...
    <main>
      ...
      <section style={{ padding: '80px 0', background: '#fff' }}>
        <div className="container-xl">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '48px', flexWrap: 'wrap', gap: '20px', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
            <div>
              <h2 style={{ fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif', fontSize: '32px', fontWeight: 800, color: '#022b63', marginBottom: '8px' }}>
                {isRTL ? 'معرض المشاريع' : 'Galerie de Projets'}
              </h2>
              <p style={{ color: '#777', fontWeight: 500, fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif' }}>{projects.length} {isRTL ? 'مشروع منجز بنجاح' : 'Projets réalisés avec succès'}</p>
            </div>
            {/* Filter Placeholder */}
            <div style={{ display: 'flex', gap: '8px', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              {[isRTL ? 'الكل' : 'Tous', 'Industriel', 'Agricole', 'Résidentiel'].map((f, i) => (
                <button key={i} style={{ padding: '8px 20px', borderRadius: '12px', border: i === 0 ? 'none' : '1px solid #EAEAEA', background: i === 0 ? '#d95015' : '#fff', color: i === 0 ? '#fff' : '#555', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif' }}>{f}</button>
              ))}
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div style={{ position: 'relative', height: '280px', overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'absolute', top: '20px', [isRTL ? 'left' : 'right']: '20px', background: 'rgba(255,255,255,0.9)', backdropFilter: 'blur(5px)', padding: '6px 14px', borderRadius: '10px', fontSize: '12px', fontWeight: 800, color: '#d95015', textTransform: 'uppercase', fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif' }}>
                      {type}
                    </div>
                  </div>
                  <div style={{ padding: '30px', textAlign: isRTL ? 'right' : 'left' }}>
                    <h3 style={{ fontSize: '20px', fontWeight: 800, marginBottom: '16px', fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif', color: '#022b63' }}>{title}</h3>
                    ...
                    <button 
                      ...
                      style={{
                        marginTop: '24px', width: '100%', padding: '12px', borderRadius: '12px', background: '#F5F5F5', border: 'none', color: '#555', fontWeight: 700, fontSize: '14px', cursor: 'pointer', transition: 'all 0.2s', fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif'
                      }} className="hover:bg-[#d95015] hover:text-white">
      ...
      <section style={{ padding: '100px 0', background: '#f8f9f7', textAlign: 'center' }}>
        <div className="container-xl">
          <div style={{ maxWidth: '700px', margin: '0 auto' }}>
            <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: '#fdf5dd', color: '#d95015', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 32px' }}>
              <Camera size={40} />
            </div>
            <h2 style={{ fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif', fontSize: '32px', fontWeight: 800, color: '#022b63', marginBottom: '20px' }}>
            ...
            <button 
              ...
              style={{ padding: '16px 40px', background: '#d95015', color: '#fff', borderRadius: '12px', border: 'none', fontSize: '16px', fontWeight: 800, cursor: 'pointer', boxShadow: '0 10px 20px rgba(217,80,21,0.2)', fontFamily: isRTL ? 'Cairo, sans-serif' : 'Alexandria, sans-serif' }}>
      ...
      {selectedProject && (
        <Modal
          ...
          title={language === 'ar' ? selectedProject.title.ar : selectedProject.title.fr}
          image={selectedProject.img}
          location={language === 'ar' ? selectedProject.location.ar : selectedProject.location.fr}
          date={selectedProject.date}
          type={language === 'ar' ? selectedProject.type.ar : selectedProject.type.fr}
          details={selectedProject.details ? (language === 'ar' ? selectedProject.details.ar : selectedProject.details.fr) : undefined}
        />
      )}
    </main>
  )
